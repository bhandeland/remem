...
from saddlebag.services.write import RuleNeedsSummary, remember, supersede, update
...
WATERMARK_NAME = ".saddlebag-sync.json"
...
class Watermark:
    entry_id: str
    body_sha: str
    #: Never compared. Shown by `bag memory status` so a stale directory is
...
#: the exact body saddlebag last wrote under the old name.
MEM_TAG_PREFIX = "mem:"
...
#: the directory scan: a sidecar is saddlebag's report of a conflict, not a
...
CONFLICT_SUFFIX = ".saddlebag-conflict.md"
...
class Report:
    ...
    #: The subset of `conflicts` that actually has a `.saddlebag-conflict.md` on
...
def _follow_renames(
    ...
    *,
    ...
    saddlebag itself last wrote under the old name, so a new file whose body
...
class AllOutcome:
    ...
    the sync - it is saddlebag declining to guess which directory a project
        ...
        if d.working_dir is None:
            out.append(
                AllOutcome(
                    ...
                    d.collection,
                    skipped=(
                        "no working directory recorded - re-run `bag memory "
    ...
    *,
    ...
    for name in sorted(set(files) | set(entries)):
        if name in unreadable:
            ...
            # signal that something happened to it which saddlebag did not do, so
        ...
        if case is Case.ADOPT_NEW:
            ...
            if not dry_run:
                ...
                # Pins stay the user's escape hatch, via `bag kb pin`.
        ...
        if case is Case.REGENERATE:
            ...
            if not dry_run:
                ...
                # An Entry has nowhere to store the metadata keys saddlebag does
                # not own, so they are read back off the file being replaced.
                # A file saddlebag creates from scratch simply has none.
    ...
    if not dry_run:
        ...
        index_path.write_text(
            memory_file.render_index(
                ...
                # A file saddlebag could not parse keeps its MEMORY.md line. The
                ...
                # Code would stop loading a file saddlebag deliberately did not
                # touch.
                carried={
                    f"{n}.md": line
                    for n, line in (
...
class Status:
    ...
    #: `<name>.saddlebag-conflict.md` sidecars still on disk from a past sync.
...
def render_run(run: MemoryRun | None) -> str:
    ...
    have to parse counts to find it. Follows `bag reingest status`.
    """
    if run is None:
        return "  never synced"
    # .astimezone() first: the column is `timestamptz` and comes back in
    # UTC, so a bare strftime prints UTC's wall clock here while `saddlebag
    ...
    # The trigger, not just the counts. A spawned `bag memory refresh`
    # and a typed `bag memory sync` leave identical rows, so a reader
...
STATUS_POINTER = "run `bag memory status --project {project}`"
...
def _as_file(
    ...
    """The file saddlebag would write for this entry.
    ...
    it would edit a field saddlebag does not own on every regenerate. An `Entry`
    also has nowhere to store the metadata keys saddlebag does not own, so those
    are read back off the file being replaced; a file saddlebag creates from
