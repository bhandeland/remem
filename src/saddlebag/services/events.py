...
from saddlebag.config import load as load_config
from saddlebag.domain import DuplicateGroup, ExtractJob, HarnessStats, ProvenanceRow
...
# it. Generous rather than unbounded: saddlebag's scale is tens of sessions
...
@dataclass(slots=True)
class StatusReport:
    """Everything `bag record status` shows, gathered in one read-only
    pass.
    ...
    #: outgrown BAG_MAX_CHARS is not an event either - but it is the one
...
def status(
    ...
    per candidate session, bounded by `STATUS_AWAITING_LIMIT` - at saddlebag's
...
def render(report: StatusReport) -> str:
    """Human-readable `bag record status`. See `to_dict` for `--json` -
    ...
    for d in report.suspected_duplicates:
        ...
        lines.append(
            ...
            f"twice; re-run `bag install {d.harness}`"
        )
...
def forensics(store: Store, owner_id: UUID, entry_id: UUID) -> list[ProvenanceRow]:
    """The forensic lookup behind `bag events show`.
