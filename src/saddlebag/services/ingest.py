"""Loading markdown documents into saddlebag as chunk-sized entries.
...
from saddlebag.embed import Embedder
from saddlebag.markdown import Chunk, split
from saddlebag.services.embed import backfill_if_pending
...
def relative_to_root(paths: list[Path], top: Path, cwd: Path) -> list[Path]:
    ...
    as typed, and `cd docs && bag ingest a.md` duplicated every chunk a
...
def ingest_file(
    ...
    *,
    ...
    if not existing and project is not None:
        # Entirely new to saddlebag under this identity. That is what a first
    ...
    for chunk in chunks:
        ...
        if current is None:
            ...
            # would churn updated_at and make `bag embed` look like it has
...
class RefreshResult:
    ...
    BAG_HOOK_DEBUG. Keeping the count and the error separate is what lets
...
def ingest_manual(
    ...
    *,
    ...
    """`ingest_paths`, bracketed by a run row - what `bag ingest` calls.
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # entries just written, and `bag embed` remains the loud way
...
STATUS_POINTER = "see: bag reingest status --project {project}"


@dataclass(slots=True)
class ProjectIngestStatus:
    """What `bag reingest status` knows about one project.
...
def status(
    ...
    *,
    ...
    """The facts behind `bag reingest status`. Read-only.
    ...
    a plain `bag ingest` inside a repository that has never designated
    ...
    for s in found:
        if not s.designations:
            ...
            lines.append(
                ...
                f"Designate it with `bag reingest designate <paths>`."
            )
...
def advisories(
    ...
    *,
    ...
    `bag record status` - the fail-loud half of a fail-soft pipeline,
    ...
    for s in status(store, owner_id, None, current_project=current_project, root=root):
        ...
        # designated projects. `bag ingest` is already fail-loud about its
        ...
        # whose pointer (bag reingest status --project X) contradicts
