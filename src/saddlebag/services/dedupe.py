...
_ORIGIN_RANK = {
    ...
    # default: a tool whose judgement saddlebag cannot vouch for, from a store
    # that is going away, should never outrank saddlebag's own extractor as the
...
def _resolve_lines(members: list[Entry]) -> list[str]:
    ...
    return [
        f"    bag dedupe resolve {e.id} --keep {keep.id}"
        for e in members
        if e.id != keep.id
    ]
...
def render(report: DedupeReport) -> str:
    ...
    if report.embedded == 0:
        ...
        # `bag doctor` - unchecked never renders as ok.
        out.append(
            f"Near-duplicates: not checked - no entries carry a vector for "
            f"{report.model}.\n  Run `bag embed` to enable this tier."
        )
    elif report.near:
        out.append(
    ...
    if missing:
        out.append(f"  {missing} entries were not compared - run `bag embed`.")
    return "\n".join(out)
...
class CannotResolve(Exception):
    ...
    for the reason `bag ingest` and `bag handoff write` are: a person is
