...
session. Nothing in saddlebag could see it.
...
`bag doctor`, its --json form, and the advisory line in
`bag record status`.

Opens no database connection. `bag doctor` must work with Postgres down,
...
from saddlebag.agents.base import HookState, UnsupportedScope

#: The scopes a sweep examines. saddlebag's, not the adapters' - the vocabulary
#: is already closed and already saddlebag-wide, since `--scope` accepts exactly
...
#: third-party adapter author to learn about, to answer a question saddlebag
...
class Verdict(StrEnum):
    ...
    #: hook still fires, because saddlebag's legacy commands are real aliases,
...
class AgentReport:
    ...
    #: where a file was read and saddlebag commands were found in it. None on
    ...
    path: Path | None = None
    #: An adapter is INSTALLED when at least one saddlebag command appears in
...
def check(
    ...
    - Sweeping, the user asked "is this installed anywhere". A scope the
      ...
      SKIP. This is what stops `bag doctor` reporting cursor "not
    ...
    - Named, the user asked a specific question. UnsupportedScope stays
      ...
      reply. Skipping instead would make `bag doctor claude-code --scope
    ...
    a broken one must never be why `bag doctor` will not run.
    """
    ...
    for name in sorted(adapters):
        ...
        # Either one escaping would take `bag doctor` down with a traceback,
        ...
        # held a saddlebag command, never per scope: an adapter absent from both
...
def failed(reports: list[AgentReport]) -> bool:
    ...
    dismisses in seconds by running one idempotent `bag install`. The
    ...
    with zero saddlebag commands is "not installed" and produces no findings at
...
def advisories(reports: list[AgentReport]) -> list[str]:
    ...
    pointer must reproduce its own finding. If the advisory said `saddlebag
    ...
    for report in reports:
        ...
        lines.append(
            ...
            f"`bag doctor {report.agent} --scope {report.scope}`"
        )
    return lines
...
def render(reports: list[AgentReport]) -> str:
    ...
    for report in reports:
        if report.verdict is Verdict.UNCHECKED:
            ...
            # buries a real breakage - possibly a bug in an adapter saddlebag
        ...
        if any(f.verdict is not Verdict.OK for f in report.findings):
            ...
            lines.append(f"  Fix: bag install {report.agent} --scope {report.scope}")
    return "\n".join(lines)
