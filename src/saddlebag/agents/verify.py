...
goes through the same `bag record event` the hook will call, not through a
...
from saddlebag.agents.base import HarnessEvent, InstallReport
from saddlebag.domain import EventKind, new_id
...
#: chose.
VERIFY_PROJECT = "__saddlebag_verify__"
...
def round_trip(agent_name: str, env: Mapping[str, str] | None = None) -> InstallReport:
    ...
    proven, and the caller (a user typing `bag verify`, or `install()` on
