...
name: bag-record
description: >
  Use when the user asks why saddlebag is not recording anything, why entries look
...
saddlebag can record raw events as you work and later extract durable entries
from them automatically, so durable knowledge survives without anyone
remembering to write it down. It is separate from the `bag` skill: that
...
That is almost always the answer to "why is saddlebag not recording here":

```bash
bag record status          # is this project enabled, and what's stuck?
bag record enable          # turn it on for the current directory
bag record disable         # turn it off again
```
...
gone quiet for `BAG_IDLE_MINUTES` (default 20) - not when it ends, because
not every harness has a session-end hook to trigger on. `bag events
process` does the extraction, run from cron or a later SessionStart:

```bash
bag events process          # extract from whatever session has gone quiet
```
...
The extraction model is pinned (`BAG_EXTRACT_MODEL`, default `sonnet`)
...
They do appear in `bag search` and `saddlebag recall`. So an extracted entry
...
- Nothing recorded, project not enabled - say so and offer `bag record
...
## If they still type `bag capture ...`

`bag capture enable|disable|status|drain` still run - they warn once to
