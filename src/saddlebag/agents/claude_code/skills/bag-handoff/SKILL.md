...
description: >
  ...
  saddlebag so it can be resumed after clearing.
---

# Session Handoff

Persist session state so the session can be `/clear`ed without losing anything,
then point at the resume path. `bag-handoff` -> `/clear` -> `bag-prime`
...
2. **Write the handoff.** Four sections, in this order, always all four:
   ...
   ```bash
   bag handoff write --topic gitlab-ci --body "$(cat <<'EOF'
   ## Done
   ...
   ## In flight
   ...
...
3. **Fold durable facts into saddlebag proper.** A gotcha that will still be true
   next month belongs in a note or a rule, not in a handoff that the next
   handoff supersedes:

   ```bash
   bag remember "Runner cache key must include the lockfile hash" --body "..."
   bag rule "Never rerun a release pipeline; tag a new patch instead" --body "..."
   ```

4. **Close.** Print the entry id and the exact resume path: `/clear`, then the
   bag-prime skill with this topic.
...
- Do not start new work during a handoff. If the user asks for one more thing,
  do it, then hand off.
- If `bag handoff write` fails, say so plainly and do not suggest `/clear`.
  Unlike saddlebag's hooks, this command is loud on purpose: the context is about
  to be thrown away.
