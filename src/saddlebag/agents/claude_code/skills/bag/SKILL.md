...
1. **Broad pass.** One to three keyword searches - project names, file names,
   ...
   ```bash
   bag search "connection pool" --limit 20
   ```
...
`--handoff` to search them too, or use the `bag-prime` skill to resume from
one directly.
...
```bash
bag remember "Postgres pool sizing" --body "..." --kind note --project myapp
bag remember "Never use em dashes" --body "..." --kind rule --project myapp
```
...
```bash
bag supersede <id> --title "New title" --body "What is true now"
```

## Settings

`bag config list` shows every saddlebag and Claude Code setting, its value, and
whether it came from the environment, a file, or a default.

```bash
bag config set BASH_DEFAULT_TIMEOUT_MS 10m
bag config unset DISABLE_TELEMETRY
```

If the user asks why a setting they changed had no effect, run `bag config
get <key>` - a `source` of `environment` on a saddlebag key means an export is
shadowing the file.
