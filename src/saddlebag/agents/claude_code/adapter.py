"""Installs saddlebag into Claude Code: MCP server, hooks, and bundled skills."""
...
from saddlebag.agents.base import (
    RECORD_NOTE,
    EnvVar,
    ExpectedHook,
    HarnessEvent,
...
from saddlebag.agents.verify import VERIFY_PROJECT, round_trip
from saddlebag.domain import EventKind
from saddlebag.project import resolve_project

HOOK_COMMAND = "bag hook session-start"
RECORD_EVENT_COMMAND = "bag hook record-event"
SESSION_SIZE_COMMAND = "bag hook session-size"

#: Commands a previous saddlebag wrote for a hook, which this install migrates in
#: place. `bag hook session-end` predates the idle trigger and survives as a
...
SLUG_CONVENTION = (
    ...
    "session's repository name - create one with `bag kb new <repo-name>`. "
    "A subdirectory or a worktree resolves to the same name."
)

HANDOFF_NOTE = (
    "Long sessions get a handoff reminder at 150 turns, then every 50 - "
    "tune it with BAG_TURN_WARN_AT and BAG_TURN_WARN_EVERY."
)

CONFIG_NOTE = (
    "Tune saddlebag and Claude Code together with `bag config list` - it shows "
...
class ClaudeCodeAdapter:
    ...
    def _install_hook(
        ...
        for entry in HOOK_ENTRIES:
            ...
            # Normalise saddlebag's own entries before testing membership, so a
            # file naming only the old command upgrades rather than
            # accumulating, and one naming both collapses. Scoped to the
            # commands saddlebag writes: settings.json is shared, and an install
            ...
            for group in groups:
                ...
                for h in group.get("hooks", []):
                    if h.get("command") not in ours:
                        ...
                        continue
                    if seen:
                        # A second saddlebag entry on this event fires a second
    ...
    def _install_skill(self, paths: ClaudePaths, report: InstallReport) -> None:
        ...
        """
        root = paths.skills
        source_root = resources.files("saddlebag.agents.claude_code") / "skills"
        for skill_dir in sorted(source_root.iterdir(), key=attrgetter("name")):
            ...
            # so this conversion is exact; a zipimported saddlebag would need
    ...
    def settings_path(self, home: Path, env: Mapping[str, str]) -> Path:
        """Where `bag config` writes this agent's env block.
    ...
    def env_settings(self) -> Mapping[str, EnvVar]:
        """The environment variables `bag config` may write for this agent.
    ...
    def verify(
        ...
        `saddlebag.agents.verify.round_trip`, shared with every adapter.
        """
    ...
    def hook_state(
        ...
        `read_document`. Unreadable settings read as no saddlebag hooks at all,
        ...
        for entry in HOOK_ENTRIES:
            # Legacy commands count as found. `bag hook session-end` runs
