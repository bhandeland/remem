"""The Claude Code environment variables `bag config` may set.

This table lives on the adapter, not in services/, because
BASH_DEFAULT_TIMEOUT_MS is a fact about Claude Code rather than about saddlebag.
...
prevent. A future adapter ships its own table and `bag config` works for it
...
Curated rather than passthrough: the help strings make `bag config list`
...
#   credential - saddlebag must never be the tool that writes a secret into a
#   JSON file on disk.
#
