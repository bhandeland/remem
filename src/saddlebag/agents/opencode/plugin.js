// generated by saddlebag - do not edit. `bag install opencode`
// overwrites this file unconditionally.
//
// A marshaller and nothing else: it shapes opencode's hook arguments into
// JSON and hands them to the `bag` CLI, which decides everything. It
...
// cancelled, so a hung `bag` may outlive this call. That is deliberate.
...
async function callSaddlebag($, args, stdinData, timeoutMs) {
  let timer;
  try {
    const stdinText = JSON.stringify(stdinData);
    const running = $`bag ${args} < ${new Response(stdinText)}`
      .quiet()
      .nothrow();
    ...
    // shell failure that rejects anyway (e.g. `bag` missing from PATH).
    ...
    // pretends this makes the underlying `bag` process stop running.
    running.catch(() => {});
...
export const server = async ({ $, directory, worktree }) => {
  ...
  // where the session was started. Either is fine - saddlebag resolves the
  ...
  return {
    ...
    "experimental.chat.system.transform": async (input, output) => {
      ...
      const block = await callSaddlebag(
        $,
