...
from saddlebag.project import repo_root
...
def plugin_dir(scope: str, home: Path, cwd: Path) -> Path:
    ...
    # honouring it in saddlebag's installer keeps both tools pointed at the same
    # place. opencode has no documented equivalent - inventing one would
    # give saddlebag a knob opencode itself does not read, which is worse than
