...
That makes this the first place saddlebag writes context into the user's
...
RULES_PATH = ".cursor/rules/saddlebag.mdc"
...
#: `.cursor/rules/saddlebag.mdc` is harmless: saddlebag is the only thing that
...
#: this, a repository excluded by an older saddlebag would keep the old,
...
_FRONTMATTER = """\
---
description: saddlebag knowledge base
alwaysApply: true
---

<!-- generated by saddlebag - do not edit. Rewritten at every session start. -->
...
def write(root: Path, block: str) -> Path:
    ...
    Machine-owned, like opencode's saddlebag.js: no merge, no version marker,
...
def _git_common_dir(root: Path) -> Path | None:
    ...
    that layout is git's to define, not ours to guess). saddlebag's own
...
def exclude(root: Path) -> bool:
    ...
    # would treat a *commented-out* `# **/.cursor/rules/saddlebag.mdc` as already
    ...
    if _LEGACY_EXCLUDE_PATTERN in live_lines:
        # A pre-fix saddlebag wrote the bare, root-anchored line here. Drop it
