"""Where Cursor's hooks.json lives, and how saddlebag merges into it."""
...
from saddlebag.agents.base import ExpectedHook, UnsupportedScope
...
#:
#: `bag record event --agent cursor`, NOT `bag hook record-event` -
...
HOOK_ENTRIES: tuple[CursorHook, ...] = (
    CursorHook(
        "sessionStart",
        "bag hook context --agent cursor",
        True,
        "context injection, and the only extraction trigger Cursor has",
    ),
    CursorHook(
        "postToolUse", "bag record event --agent cursor", True, "every tool call"
    ),
    CursorHook(
        "beforeSubmitPrompt",
        "bag record event --agent cursor",
        True,
...
def hooks_path(scope: str, home: Path, cwd: Path) -> Path:
    ...
    harness will never look. So `bag install cursor --scope project` and
    `bag doctor` both behave differently here depending on which
...
def merge(
    ...
    Merged rather than overwritten, unlike opencode's saddlebag.js: hooks.json
    is user-owned and other tools legitimately write to it, so saddlebag adds
    ...
    for hook, command in entries.items():
        ...
        if not isinstance(group, list):
            group: list[Any] = []
            hooks[hook] = group
        # Repair saddlebag's own entries before testing membership. Idempotence
        ...
        # naming a command saddlebag has since renamed. A hook that fires twice
        # doubles every row it records.
        #
        # Scoped to the commands saddlebag writes: hooks.json is shared and
