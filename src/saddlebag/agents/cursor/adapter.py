"""Installs saddlebag into Cursor: hook entries merged into hooks.json, and a
generated rules file.

The contrast with the other two adapters is the point. opencode gets one
file saddlebag owns outright; Claude Code gets entries merged into two JSON
...
`.mdc` is machine-owned and overwritten, because saddlebag is the only thing
that ever writes it.
"""
...
class CursorAdapter:
    ...
    def identity(self, env: Mapping[str, str], payload: dict[str, Any]) -> Identity:
        return Identity(
            ...
            # every other write path in saddlebag.
    ...
    def inject(
        ...
        frontend decide (BAG_HOOK_DEBUG for the CLI, a list's `append`
        ...
        """
        from saddlebag.agents.cursor import rules
    ...
    def install(
        ...
        """Merge saddlebag's hook entries into hooks.json, then verify.
        ...
        """
        from saddlebag.agents.cursor.install import ENTRIES, hooks_path, merge
        ...
        report.notes.append(
            "The knowledge base block is written to .cursor/rules/saddlebag.mdc "
    ...
    def verify(
        ...
        """
        from saddlebag.agents.verify import round_trip
    ...
    def hook_state(
        ...
        `read_json`. Unreadable JSON reads as no saddlebag hooks - the honest
        ...
        """
        from saddlebag.agents.cursor.install import (
            HOOK_ENTRIES,
            LEGACY_COMMANDS,
            hooks_path,
        )
