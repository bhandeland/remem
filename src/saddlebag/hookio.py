...
`bag record event` CLI command, which is a hook entry point in everything
...
def debug(env: Mapping[str, str], reason: str) -> None:
    """Explain an empty result, but only when BAG_HOOK_DEBUG is set.
    ...
    """
    try:
        if env.get("BAG_HOOK_DEBUG"):
            sys.stderr.write(f"bag hook: {reason}\n")
    except Exception:
        pass


def spawn_process(env: Mapping[str, str]) -> bool:
    """Start a detached `bag events process` and return immediately.
    ...
    it through `bag hook context`. One trigger all three share beats a
    ...
    """
    return _spawn(["bag", "events", "process"], env)


def spawn_ingest(env: Mapping[str, str]) -> bool:
    """Start a detached `bag reingest run` and return immediately.
    ...
    """
    return _spawn(["bag", "reingest", "run"], env)


def spawn_memory(env: Mapping[str, str]) -> bool:
    """Start a detached `bag memory refresh` and return immediately.

    The third spawn, from the same two places as the other two - Claude
    Code's SessionStart and `bag hook context` - because that pair is the
    ...
    `bag memory refresh`, not `bag memory sync`: sync is the command a
    ...
    its sidecar and says nothing; `bag memory status` and the advisory
    line in `bag record status` are what surface it afterwards.
    """
