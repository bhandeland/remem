...
#: is what keeps `saddlebag` installable on a laptop without PyTorch.
...
class EmbedderUnavailable(RuntimeError):
    ...
    Raised rather than degraded, because the caller is `bag embed`, whose
...
class LocalEmbedder:
    ...
    noticeable moment to import. Every `bag` invocation would pay that -
    ...
    deferred to the point of use, which is `bag embed` and the semantic
    ...
    def __init__(self, model_name: str = DEFAULT_EMBED_MODEL) -> None:
        ...
        try:
            ...
            raise EmbedderUnavailable(
                ...
                "If this is a name typo, fix BAG_EMBED_MODEL; if it is a "
                "download failure, re-run `bag embed` when online. "
