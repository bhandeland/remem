...
# harnesses saddlebag targets do not have one - and because a hook that never
...
def default_config_path() -> Path:
    return user_config_path("saddlebag") / "config.toml"
...
def _read_file(path: Path) -> dict[str, Any]:
    try:
        ...
        # A broken or missing config file must never stop saddlebag from running.
        return {}
...
def load(
    ...
    path = (
        ...
        else Path(env.get("BAG_CONFIG", default_config_path()))
    )
    ...
    max_chars = _as_int(
        pick("BAG_MAX_CHARS", "max_chars", DEFAULT_MAX_CHARS),
        DEFAULT_MAX_CHARS,
    )

    threshold = _as_float(
        pick("BAG_FUZZY_THRESHOLD", "fuzzy_threshold", DEFAULT_FUZZY_THRESHOLD),
    ...
    # BAG_CAPTURE_MODEL is read for one release, and warns. The two silent
    ...
    if "BAG_EXTRACT_MODEL" in env:
        extract_model = str(env["BAG_EXTRACT_MODEL"])
    elif "BAG_CAPTURE_MODEL" in env:
        sys.stderr.write(
            "BAG_CAPTURE_MODEL is renamed to BAG_EXTRACT_MODEL; "
            "set BAG_EXTRACT_MODEL instead.\n"
        )
        extract_model = str(env["BAG_CAPTURE_MODEL"])
    ...
    turn_warn_at = positive_int(
        "BAG_TURN_WARN_AT", "turn_warn_at", DEFAULT_TURN_WARN_AT
    )
    turn_warn_every = positive_int(
        "BAG_TURN_WARN_EVERY", "turn_warn_every", DEFAULT_TURN_WARN_EVERY
    )
    ...
    idle_minutes = positive_int(
        "BAG_IDLE_MINUTES", "idle_minutes", DEFAULT_IDLE_MINUTES
    )

    embed_model = str(
        pick("BAG_EMBED_MODEL", "embed_model", DEFAULT_EMBED_MODEL)
    ).strip()
    if not embed_model:
        embed_model = DEFAULT_EMBED_MODEL

    semantic = pick(
        "BAG_SEMANTIC_THRESHOLD", "semantic_threshold", DEFAULT_SEMANTIC_THRESHOLD
    )
    ...
    return Config(
        ...
        dsn=str(pick("BAG_DSN", "dsn", DEFAULT_DSN)),
        user_handle=str(pick("BAG_USER_ID", "user_handle", DEFAULT_HANDLE)),
        max_chars=max_chars,
