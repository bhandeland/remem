...
#: saddlebag's prose style. The only place in the repo where that is true.
INDEX_SEPARATOR = " — "

_INDEX_LINE = re.compile(r"^- \[(?P<title>.+?)\]\((?P<file>[^)]+)\)")

#: The frontmatter keys saddlebag itself understands and rebuilds from an entry.
...
class MemoryFile:
    ...
    type: str | None
    body: str
    #: Metadata keys saddlebag does not own, preserved verbatim and in source
    ...
    #: them would destroy provenance on a file saddlebag did not write.
...
def index_lines(text: str) -> dict[str, str]:
    ...
    a file saddlebag could not parse and therefore has no entry to rebuild from.
    """
    out: dict[str, str] = {}
    for line in text.splitlines():
        stripped = line.strip()
...
def parse(text: str, *, name: str, title: str) -> MemoryFile:
    ...
    fields: dict[str, str] = {}
    # Metadata keys other than `type`, in the order they appeared. saddlebag
    ...
    for line in head.splitlines():
        ...
        if in_metadata and line.startswith(" "):
            ...
            if key in _OWN_KEYS:
                ...
                # therefore what saddlebag last agreed to.
                fields.setdefault("metadata.type", value)
            else:
                # Anything else at the top level is bookkeeping saddlebag does
                # not own, and lands in `extra` for exactly the reason the
                # indented keys do: dropping it destroys provenance on a
                # file saddlebag did not write. It comes back indented, because
