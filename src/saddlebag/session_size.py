...
def state_path() -> Path:
    return user_cache_path("saddlebag") / "session-size.json"
...
def reminder(count: int) -> str:
    return (
        ...
        f"mid-task - offer the user a handoff: run the bag-handoff skill, "
        f"then /clear, then bag-prime to resume. Do not interrupt work in "
        f"progress to do it."
    )
