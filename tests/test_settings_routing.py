...
def test_a_remem_key_routes_to_remems_own_config() -> None:
...
def test_write_remem_creates_the_file_with_the_unprefixed_key(tmp_path: Path) -> None:
    path = tmp_path / "config.toml"
    write_remem(path, "REMEM_MAX_CHARS", "8000")
    assert tomllib.loads(path.read_text())["max_chars"] == 8000


def test_write_remem_preserves_other_settings(tmp_path: Path) -> None:
    path = tmp_path / "config.toml"
...
def test_write_remem_round_trips_a_dsn_with_quotes_and_backslashes(
    tmp_path: Path,
) -> None:
...
def test_write_remem_backs_up_before_rewriting(tmp_path: Path) -> None:
...
def test_write_remem_unsets_with_none(tmp_path: Path) -> None:
    path = tmp_path / "config.toml"
