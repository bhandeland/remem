...
def _live(store: PostgresStore, owner: Principal) -> list[Hit]:
    return store.search(
        Query(tags=["cmem:m1"], origins=[Origin.IMPORTED], limit=50), owner.id
    )


def test_an_import_writes_an_entry_with_the_imported_origin(
...
def test_importing_the_same_record_twice_writes_nothing_the_second_time(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_a_changed_record_supersedes_rather_than_duplicating(
    store: PostgresStore, owner: Principal
) -> None:
    run(store, owner.id, [_record()], namespace="cmem")
...
def test_the_replacement_keeps_the_identity_tag(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_a_second_run_with_a_different_project_does_not_move_the_entry(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_a_second_run_with_a_different_project_reports_where_entries_land(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_an_unchanged_second_run_also_reports_the_entrys_real_project(
    store: PostgresStore, owner: Principal
) -> None:
    run(store, owner.id, [_record()], namespace="cmem", project="at-workspace")
...
def test_a_record_with_no_project_lands_in_the_no_project_bucket(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_a_second_live_hit_on_one_identity_tag_is_refused(
    ...
    def two_hits(*args: Any, **kwargs: Any) -> list[Hit]:
...
def test_a_dry_run_writes_nothing(store: PostgresStore, owner: Principal) -> None:
...
def test_a_dry_run_counts_by_kind_and_project(
    store: PostgresStore, owner: Principal
) -> None:
    records = [
        _record(),
...
def test_a_deleted_source_row_never_removes_an_entry(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_a_skipped_list_passed_in_arrives_on_the_report(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_a_schema_without_the_imported_origin_is_refused_by_name(
    conn: psycopg.Connection[Any], owner: Principal
) -> None:
...
def test_a_non_schema_probe_failure_surfaces_its_own_error(
    store: PostgresStore, owner: Principal, monkeypatch: pytest.MonkeyPatch
) -> None:
