...
def test_remember_defaults_to_a_memory_from_an_agent(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_remember_records_provenance(store: PostgresStore, owner: Principal) -> None:
    e = write.remember(
        store,
        owner.id,
        title="T",
...
def test_update_changes_only_what_is_given(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_supersede_creates_a_new_entry_and_marks_the_old(
    store: PostgresStore, owner: Principal
) -> None:
    old = write.remember(
        store,
...
def test_supersede_inherits_project_and_tags(
    store: PostgresStore, owner: Principal
) -> None:
    old = write.remember(
        store,
...
def test_superseded_entry_disappears_from_search(
    store: PostgresStore, owner: Principal
) -> None:
...
def test_link_is_bidirectional_and_deduped(
    store: PostgresStore, owner: Principal
) -> None:
    ...
    write.link(store, owner.id, a.id, b.id)
    assert found(store.get_entry(a.id, owner.id)).links == [b.id]
    assert found(store.get_entry(b.id, owner.id)).links == [a.id]


def test_nothing_is_hard_deleted_by_supersede(
    store: PostgresStore, owner: Principal
) -> None:
