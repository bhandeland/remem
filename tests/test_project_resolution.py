...
def test_the_repository_root_resolves_to_the_repository_name(repo: Path) -> None:
    assert resolve_project(repo) == "myrepo"


def test_a_subdirectory_resolves_to_the_repository_not_the_subdirectory(
    repo: Path,
) -> None:
...
def test_a_worktree_resolves_to_the_main_repository(repo: Path, tmp_path: Path) -> None:
...
def test_outside_a_repository_it_falls_back_to_the_directory_name(
    tmp_path: Path,
) -> None:
...
def test_a_git_failure_falls_back_rather_than_raising(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    def boom(*args: Any, **kwargs: Any) -> subprocess.CompletedProcess[Any]:
...
def test_the_adapter_resolves_the_payload_cwd_through_the_repository(
    repo: Path,
) -> None:
...
def test_the_filesystem_root_yields_no_project(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
