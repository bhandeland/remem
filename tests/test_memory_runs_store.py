...
def test_a_started_run_has_no_finish(store: PostgresStore) -> None:
...
def test_finishing_records_every_count_and_list(store: PostgresStore) -> None:
    ...
    latest = found(store.latest_memory_run(owner.id, "p"))
...
def test_latest_is_the_newest_row(store: PostgresStore) -> None:
    ...
    assert found(store.latest_memory_run(owner.id, "p")).id == second.id


def test_latest_is_none_when_a_project_never_ran(store: PostgresStore) -> None:
    owner = store.ensure_principal("runs-none")
    assert store.latest_memory_run(owner.id, "never") is None


def test_runs_are_per_project(store: PostgresStore) -> None:
    ...
    assert found(store.latest_memory_run(owner.id, "one")).id == a.id
    assert found(store.latest_memory_run(owner.id, "two")).id == b.id


def test_another_principals_run_is_never_returned(store: PostgresStore) -> None:
...
def test_finishing_another_principals_run_is_refused(store: PostgresStore) -> None:
    ...
    assert found(store.latest_memory_run(theirs.id, "p")).finished_at is None
