...
def test_parses_a_well_formed_array() -> None:
    raw = """[
...
def test_an_empty_array_is_valid_and_yields_nothing() -> None:
    """Most sessions contain nothing durable. That is success, not failure."""
    assert parse_entries("[]") == []


def test_tolerates_prose_around_the_json() -> None:
...
def test_tolerates_a_fenced_code_block() -> None:
    raw = '```json\\n[{"title":"T","body":"B","kind":"doc"}]\\n```'
    assert [e.kind for e in parse_entries(raw)] == [Kind.DOC]


def test_missing_tags_defaults_to_empty() -> None:
    assert parse_entries('[{"title":"T","body":"B","kind":"note"}]')[0].tags == []


def test_drops_entries_missing_required_fields_without_failing_the_batch() -> None:
    raw = """[
...
def test_drops_entries_with_an_unknown_kind() -> None:
    raw = """[
      {"title": "Good", "body": "B", "kind": "note"},
...
def test_truncates_to_the_entry_cap() -> None:
    raw = (
        "["
...
def test_caps_title_and_body_length() -> None:
...
def test_non_string_tags_are_dropped() -> None:
...
def test_unparseable_output_raises_extraction_failed(raw: str) -> None:
    with pytest.raises(ExtractionFailed):
        parse_entries(raw)


def test_prefers_the_array_that_actually_contains_entries() -> None:
...
def test_tolerates_a_bracket_in_prose_after_a_fenced_block() -> None:
    raw = (
        '```json\n[{"title":"Good","body":"B","kind":"note"}]\n```\n'
...
def test_an_empty_array_still_wins_over_no_valid_entries() -> None:
    """[] is the deliberate 'nothing durable' answer and stays a success."""
    assert parse_entries("Nothing to record: []") == []


def test_whitespace_only_title_or_body_is_dropped() -> None:
...
def test_failure_carries_the_raw_output_for_diagnosis() -> None:
...
def test_failure_on_a_valueless_array_carries_the_raw_output() -> None:
    with pytest.raises(ExtractionFailed) as exc:
        parse_entries("[1, 2, 3]")
