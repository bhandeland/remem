...
@pytest.mark.db
def test_remember_defaults_project_to_the_directory(
...
@pytest.mark.db
def test_global_writes_an_entry_with_no_project(
    env: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
@pytest.mark.db
def test_global_and_project_together_are_refused(env: str) -> None:
...
@pytest.mark.db
def test_rule_accepts_tags_and_global(
    env: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_an_empty_editor_body_aborts(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.db
def test_mcp_remember_defaults_project_to_the_directory(
    env: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
@pytest.mark.db
def test_mcp_remember_honours_an_explicit_project(
    env: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.chdir(tmp_path)
    from saddlebag.mcp_server import remember_tool
