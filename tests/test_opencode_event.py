...
def test_tool_call_becomes_a_tool_call_event(repo: Path) -> None:
...
def test_chat_message_becomes_a_message_event(repo: Path) -> None:
    payload = {
        "hook": "chat.message",
        "sessionID": "ses_abc123",
        "cwd": str(repo),
...
def test_the_whole_payload_is_carried_through_unparsed(repo: Path) -> None:
...
def test_an_unsubscribed_hook_records_nothing(repo: Path) -> None:
    payload = {"hook": "chat.params", "sessionID": "ses_abc123", "cwd": str(repo)}

    assert OpenCodeAdapter().event({}, payload) is None


def test_a_payload_with_no_session_id_records_nothing(repo: Path) -> None:
