...
def test_the_fixture_corpus_is_actually_there() -> None:
...
def test_body_round_trips_byte_for_byte(path: Path):
    # The sync's watermark hashes the body alone (frontmatter is
    # deliberately excluded, so YAML whitespace can never read as a
    # content change) -> None - so this is the property that actually has to hold.
...
@pytest.mark.parametrize("path", _fact_files(), ids=lambda p: p.name)
def test_render_is_idempotent(path: Path):
    # A file with non-canonical frontmatter (key order, a stray trailing
    # space) -> None is normalised once, on first write, and must be stable after
...
@pytest.mark.parametrize("path", _fact_files(), ids=lambda p: p.name)
def test_no_metadata_key_is_lost(path: Path) -> None:
    text = path.read_text()
    mf = _parse_fixture(path)
    rendered = memory_file.render(mf)
    in_metadata = False
...
def test_parse_rejects_a_file_with_no_frontmatter() -> None:
    with pytest.raises(memory_file.MalformedMemoryFile):
        memory_file.parse("just a body", name="x", title="X")


def test_parse_index_maps_filename_to_link_text() -> None:
    index = memory_file.parse_index((FIXTURES / "MEMORY.md").read_text())
...
def test_render_index_is_sorted_by_name_and_uses_the_em_dash() -> None:
    files = [
...
def test_title_from_name_is_mechanical() -> None:
    assert memory_file.title_from_name("cursor-runs-claude-code-hooks") == (
        "Cursor runs claude code hooks"
    )


def test_body_sha_ignores_nothing_and_is_stable() -> None:
...
def test_a_top_level_bookkeeping_key_survives_a_render() -> None:
    mf = _parse_fixture(FIXTURES / "flat-frontmatter-dialect.md")
...
def test_the_two_dialects_parse_to_the_same_shape():
    ...
    # must not be observable downstream of parse() -> None.
