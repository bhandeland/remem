...
from saddlebag.agents.opencode.hooks import (
    HOOK_NAMES,
...
def test_the_vendored_list_still_matches_the_installed_types():
    ...
    assert installed == HOOK_NAMES, (
        ...
        "PLUGIN_TYPES_VERSION in src/saddlebag/agents/opencode/hooks.py, then "
...
def _plugin_source() -> str:
    ...
    return (
        resources.files("saddlebag.agents.opencode").joinpath("plugin.js").read_text()
    )
...
def test_the_plugin_says_it_is_generated():
    assert _plugin_source().startswith("// generated by saddlebag - do not edit")
...
def test_the_adapter_records_only_hooks_opencode_emits():
    ...
    None, and `bag record event` exits 0 having recorded nothing.
    """
...
def test_plugin_serializes_inside_try_catch():
    ...
    callSaddlebag's try block, or a circular reference or BigInt in tool output
    ...
    in the plugin, within the callSaddlebag function's try block, never as an
    argument expression to callSaddlebag in any of the three handlers.
    """
    source = _plugin_source()
    lines = source.split("\n")

    # Find callSaddlebag function boundaries
    callsaddlebag_start = None
    callsaddlebag_end = None

    for i, line in enumerate(lines):
        if "async function callSaddlebag" in line:
            callsaddlebag_start = i
        if callsaddlebag_start is not None and callsaddlebag_end is None:
            if line.strip().startswith("}") and i > callsaddlebag_start:
                callsaddlebag_end = i
                break

    assert callsaddlebag_start is not None and callsaddlebag_end is not None, (
        "Could not find callSaddlebag function span - test is broken"
    )
    ...
    assert stringify_calls == 1, (
        ...
        "callSaddlebag's try block, never as a hook argument expression."
    )

    # Verify it's within callSaddlebag
    callsaddlebag_text = "\n".join(lines[callsaddlebag_start : callsaddlebag_end + 1])
    assert "JSON.stringify" in callsaddlebag_text, (
        "JSON.stringify(...) found outside callSaddlebag function. "
