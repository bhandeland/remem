...
def test_an_extracted_rule_is_written_without_a_summary(store, owner):
    ...
    report = extraction.process(store, owner.id, extractor, idle_seconds=IDLE, limit=10)
...
def test_a_second_run_extracts_nothing_new(store, owner):
    ...
    first = extraction.process(store, owner.id, extractor, idle_seconds=IDLE, limit=10)
    second = extraction.process(store, owner.id, extractor, idle_seconds=IDLE, limit=10)
...
def test_an_extractor_that_raises_fails_the_job_and_records_why(store, owner):
    ...
    report = extraction.process(
        store, owner.id, ProseExtractor(), idle_seconds=IDLE, limit=10
    )
...
def test_the_attempt_cap_stops_a_job_and_process_job_overrides_it(conn, store, owner):
    ...
    report = extraction.process_job(
        store, owner.id, job.id, FakeExtractor([an_entry()])
    )
...
def test_a_dead_session_does_not_crowd_out_a_live_one(conn, store, owner):
    ...
    report = extraction.process(store, owner.id, good, idle_seconds=IDLE, limit=1)
...
def test_an_entry_the_project_already_holds_is_not_rewritten(store, owner):
    ...
    report = extraction.process(
        store, owner.id, FakeExtractor([entry]), idle_seconds=IDLE, limit=10
    )
    ...
    titles = [h.entry.title for h in store.search(Query(text="Pool sizing"), owner.id)]
...
def test_dedup_does_not_block_the_same_title_in_another_project(store, owner):
    ...
    three_events(
        store, owner, project="beta", session_id="b", base=NOW - timedelta(hours=3)
    )
...
def test_one_failing_session_does_not_stop_the_others(store, owner):
    ...
    report = extraction.process(
        store, owner.id, HalfBroken(), idle_seconds=IDLE, limit=10
    )
...
def test_the_limit_bounds_one_run(store, owner):
    ...
    report = extraction.process(
        store, owner.id, FakeExtractor([]), idle_seconds=IDLE, limit=2
    )
...
def test_repeated_success_never_exhausts_the_attempt_budget(conn, store, owner):
    ...
    for i in range(rounds):
        ...
        report = extraction.process(
            store,
            owner.id,
            FakeExtractor([an_entry(title=f"round {i}")]),
            idle_seconds=IDLE,
            limit=10,
        )
        assert (report.claimed, report.succeeded, report.failed) == (1, 1, 0), (
