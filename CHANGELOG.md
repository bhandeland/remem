...
Notable changes to saddlebag. Versions follow [semantic versioning](https://semver.org),
...
- `bag dedupe report` for entries that say the same thing twice, and
  `bag dedupe resolve` to point one at another. Nothing is ever merged
  automatically.
...
- `bag record status` now reports a knowledge base whose rules are
...
- Adapters for Claude Code, opencode, and Cursor, registered through the
  `saddlebag.agents` entry point and loaded lazily. A broken third-party adapter
  warns rather than breaking saddlebag.
- Per-tool-call event recording, opt-in per project, extracted into entries
  later by an idle-triggered background pass.
- `bag doctor` to answer whether the installed configuration actually
...
- `bag verify` to answer whether saddlebag works when called. doctor and verify
  are a pair; neither subsumes the other.

### Documents

- `bag ingest` for markdown, one entry per heading plus an anchor per file,
  idempotent on re-ingest. Automatic re-ingest for designated paths.
- `bag memory sync` to own Claude Code's file-based memory directory as a
  generated view of a collection, adopting anything on disk it has not seen
  before it regenerates.
- `bag import claude-mem` to read claude-mem's sqlite file directly,
  reading both its pre-33 and modern schemas and naming every row it could
  not carry across.

### Interfaces

- `bag`, a Typer CLI, with `--json` on the commands worth scripting.
...
- `bag config` to read and write saddlebag's own settings and a curated set of
...
- Postgres 18 with pgvector, behind a `Store` protocol. Numbered forward-only
  migrations, applied by `bag db up` and never by anything else.
